...
namespace P2P {
	...
	void P2PLobby::handleTeamFightEndMatch() {
		...
		if (IsHost() && CheckIfEveryoneHasState(State::TeamFightEndMatch)) {
			Seamless::ForceUseSeamlessItem(Seamless::SEPERATION_MIST_REF);
		}
	}
	...
	void P2PLobby::handleBrawlInitiate() {
		...
		if (isHost && isEveryoneReady) {
			_brawlManager->PrepareMap();
	...
	void P2PLobby::handleBrawlEndMatch() {
		...
		if (IsHost() && CheckIfEveryoneHasState(State::BrawlEndMatch)) {
			Seamless::ForceUseSeamlessItem(Seamless::SEPERATION_MIST_REF);
		}
	}
