...
public:
    ...
    typedef void RemoveSpeffectFunction(uint8_t* playerIns, uint32_t speffectId);
...
inline void SPeffect::AddSpeffect(uint32_t spEffectId, uint8_t playerIndex) {
    ...
    if (playerIns == nullptr || addSpeffectFunction == nullptr) {
        printf("Something went wrong. Couldn't find the addSpeffectFunction!\n");
        return;
    }
...
inline void SPeffect::RemoveSpEffect(uint32_t spEffectId, uint8_t playerIndex) {
    ...
    if (removeSpeffectFunction == nullptr) {
        printf("Something went wrong. Couldn't find the removeSpeffectFunction!\n");
        return;
    }
...
inline bool SPeffect::HasSpeffect(uint32_t spEffectId, uint8_t playerIndex) {
    ...
    if (playerIns == nullptr) {
        // If the player instance is null, return false
        return false;
    }
